...
public class Collection_list extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lv_collection= (PullToRefreshListView) findViewById(R.id.lv_collection);
        ...
        searchView= (SearchView)findViewById(R.id.search_collection);
        searchView.setIconifiedByDefault(false);
        searchView.setSubmitButtonEnabled(true);
        searchView.setQueryHint("在“我的收藏”中搜索");


     /***************************************************************************************/

//        String sq="select * from notes where is_collect=?";
//        Cursor c=dbread.rawQuery(sq,new String[]{"true"});
//        adapter=new Adapter(this,R.layout.list_cell_item,c);
//        lv_collection.setAdapter(adapter);

        lv_collection.setOnItemClickListener(new AdapterView.OnItemClickListener() {//点击一个Item时，查询数据库，发送消息
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (!passwd.equals("")) {
                    AlertDialog.Builder dialog = new AlertDialog.Builder(Collection_list.this);
                    ...
                    dialog.setPositiveButton("确定", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            if (passwd.equals(input_passwd)) {
                                ...
                                Intent i = new Intent(Collection_list.this, Edit_Text.class);
...
//                    Log.i(TAG, "要发送的content是" + c.getString(c.getColumnIndex("content")));
        ...
        ListView actualListView = lv_collection.getRefreshableView();

        actualListView.setOnItemLongClickListener(longClick);

        lv_collection.setOnRefreshListener(new PullToRefreshBase.OnRefreshListener2<ListView>() {
                                               @Override
                                               public void onPullDownToRefresh(final PullToRefreshBase<ListView> refreshView) {
                                                   lv_collection.getLoadingLayoutProxy().setRefreshingLabel("正在刷新");
                                                   lv_collection.getLoadingLayoutProxy().setPullLabel("下拉刷新数据");
                                                   lv_collection.getLoadingLayoutProxy().setReleaseLabel("释放开始刷新");
                                                   //模拟加载数据线程休息3秒
                                                   new AsyncTask<Void, Void, Void>() {
                                                       ...
                                                       protected void onPostExecute(Void result) {
                                                           ...
                                                           //将下拉视图收起
                                                           lv_collection.onRefreshComplete();
                                                       }
                                                   }.execute();
                                               }
                                               ...
                                               public void onPullUpToRefresh(PullToRefreshBase<ListView> refreshView) {
                                                   lv_collection.getLoadingLayoutProxy().setRefreshingLabel("正在加载");
                                                   lv_collection.getLoadingLayoutProxy().setPullLabel("上拉加载更多");
                                                   lv_collection.getLoadingLayoutProxy().setReleaseLabel("释放开始加载");
                                                   //模拟加载数据线程休息3秒
                                                   new AsyncTask<Void, Void, Void>() {
                                                       @Override
                                                       protected Void doInBackground(Void... params) {
...
//                                                adapter1.notifyDataSetChanged();
        ...
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextSubmit(String query) {
                String sq="select *from notes where is_collect=? and( date like? or name like?or summary like ?)";
                Cursor c = dbread.rawQuery(sq, new String[]{"true","%" +selectWord+ "%","%" +selectWord+ "%","%" +selectWord+ "%"});
                adapter=new Adapter(Collection_list.this,R.layout.list_cell_item,c);
                lv_collection.setAdapter(adapter);
...
//        getMenuInflater().inflate(R.menu.menu_collection_list, menu);
//        return true;
//    }
//
//    @Override
...
//    }
    ...
    private AdapterView.OnItemLongClickListener longClick = new AdapterView.OnItemLongClickListener() {
        ...
        public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            Intent i = new Intent(Collection_list.this, SelectPicPopupWindow.class);
